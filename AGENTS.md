...
- `nuget-plugin/build.gradle.kts` (an included build) cannot read `gradle/libs.versions.toml`, so
  ...
  ([ADR-156](docs/adr/156-iasyncenumerable-to-flow.md), the version of `kotlinx-coroutines-core`
...
| `packNuget` fails partway through, and the consumer restore then fails with `NU1202 ... not compatible with net10.0` against an unrelated public `TestLibrary 1.1.0` on nuget.org | `build/FixtureVersions.props` is rewritten with the fresh immutable version **before** `packNuget` runs, not after it succeeds. When `packNuget` then crashes (e.g. an upstream export-symbol collision), the consumer still restores against that new version, finds no matching local package, and falls back to whatever `TestLibrary` happens to be published on nuget.org. The `NU1202` is a downstream echo; the real failure is the earlier Gradle crash, not a binding or restore bug. Check the Gradle log for the actual `packNuget` error first. Discovered alongside [ADR-095](docs/adr/095-static-route-overloads.md) |
