...
1. **Every forward route has a happy-path row in `LiveHandleTests`.** Eleven families today. The checklist is the `docs/topics/supported-features.md` mapping table, so completeness is checkable, not guessed: sealed classes, generics, value classes, structs, enums, nullable variants, `Result`, `Uuid`, map and set returns, nested collections, properties, extensions, overload sets. One `[Fact]` each, about a third of a second each.
...
4. **Objects, not only handles** ([ADR-121](docs/adr/121-kotlin-object-collectability-after-last-dispose.md), shipped for a plain class and the stored-callback receiver seam): a zero handle count with the object still reachable is a leak the counter cannot see. Extending per family (sealed arm, generic wrapper, suspend result, Flow item) is the open follow-up, tracked on ROADMAP.md.
