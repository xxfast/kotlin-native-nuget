...
| `interface` | ⇄ | `interface` | → `I`-prefixed, base list inherited (`IPet : INamed`), a `var` gets a setter too; a return gets a backing wrapper, a C# impl passes back · ← handle-backed Kotlin `interface` a class can implement | [Interfaces, abstract and sealed](interfaces-abstract-sealed.md) · [The bridgeable subset](bridgeable-subset.md) |
