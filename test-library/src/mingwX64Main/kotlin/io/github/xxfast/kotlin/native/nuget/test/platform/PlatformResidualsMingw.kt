...
// ADR-074 amendment fixture: the mingw actuals for `PlatformResiduals.kt`. The public surface
// here must stay IDENTICAL to `PlatformResidualsMacos.kt`; only the returned values differ
...
/** The mingw boost (24) exists only on this side, so a wrong reading means the wrong body ran. */
actual fun collarSignal(dbm: Int): Signal = if (dbm < 0) Signal.Lost else Signal.Strong(dbm + 24)
 ...
 */
internal class MingwTransponder : Transponder {
  override fun ping(): String = "pong from mingw"
}

actual fun transponder(): Transponder = MingwTransponder()
