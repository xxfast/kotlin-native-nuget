...
- [x] **Stop pinning fixture packages at `1.0.0`.** Each fixture build gives `SampleDependency` and `SampleLibrary` an immutable `1.0.0-fixture.<epoch-ms>` version, then writes `sample-app/build/FixtureVersions.props` for every fixture consumer to import. `SampleLibrary` declares the same exact version of `SampleDependency`, so both NuGet identities are fresh together. `scripts/verify-fixture-package-versioning.sh` proves two consecutive packs restore and build all consumers without clearing NuGet caches or consumer `obj`/`bin`.
