...
- Fixture builds give `SampleLibrary` and `SampleDependency` a new immutable version on every pack and write `sample-app/build/FixtureVersions.props`. The fixture consumers import that file, so they restore the exact fresh version without clearing NuGet caches or consumer `obj`/`bin`. Use `scripts/verify-fixture-package-versioning.sh` to guard this behaviour.
