 ...
 * about it. Classifying them one by one cannot cover the one that fires next, by construction: it is
 * the one nobody has classified. So the boundary is generic, and it is a *reporting* boundary rather
 * than a recovery one.
 *
 * What [guarded] changes, measured (ADR-162 spikes B and C): before it, the first such failure threw
 * out of `process()` and Gradle printed one unlocated `e: [ksp] java.lang.IllegalStateException: ...`
 * line naming one declaration, with every other offending declaration of the same surface invisible
 * until the author fixed that one and built again. After it, each failing declaration is reported as
...
internal object ForwardInternalFailures {
  ...
  fun firstReport(declaration: String, detail: String): Boolean = reported.add(declaration to detail)

  fun reset() = reported.clear()
}

/**
 * The one-line rendering of a caught failure: the exception class plus its message, which together
 * are what makes the generator bug findable in the source. The class name alone is useless (they are
 * all `IllegalStateException`) and the message alone loses the distinction between a failed
 * `require` and a null `!!`.
 */
 ...
 * (dropping the declaration from the export scope) rather than asking them to change working Kotlin.
 *
