...
- [ ] `warnDroppedForwardCallables` hardcodes the reason sentence as "its \`<REASON>\` type combination is not supported" for every `ForwardPlanSkipReason`, which reads wrong for `EXCLUDED_DEPENDENCY_TYPE` (an excluded type is fully supported, just excluded by the author's own config) and the hint text already carries the true cause. `NugetProcessor.kt`'s `warnDroppedForwardCallables`; fix is letting a reason override the sentence. Verified by this item's Tier 1 output.
- [ ] `CirClassRenderer`'s `implements` `when` orders `interfaces.isNotEmpty()` above the disposable arms, so a `CirClass` with interfaces never lists `IDisposable`/`IAsyncDisposable` in its base list although `renderDispose` still emits `Dispose()`; harmless today because the only production site with interfaces is the ADR-040 backing wrapper whose `I<Name>` interface carries the members (inferred, not verified). Verified by reading during this cleanup.
- [ ] A base class's own `open val`/`open var` never renders `virtual` in C#, since `isOpenInterfaceImplementation` (`CirClassTranslator.kt:38-39`) only fires for an `override` implementing an interface member with no exported superclass; a plain `open` declaration's own modifier is never read. Any subclass `override` of it is `CS0506`. Verified by reading. Discovered alongside [ADR-075](docs/adr/075-collection-property-getter-setter-independence.md)'s 2026-09-07 amendment.
- [ ] An unimplemented `abstract val`/`abstract var` on an exported base class has no abstract-property path in `CirClassTranslator.kt` at all (only `abstractMethods` exists, `:534-536`), so a subclass `override` of it is `CS0115`. Verified by reading. Discovered alongside [ADR-075](docs/adr/075-collection-property-getter-setter-independence.md)'s 2026-09-07 amendment.
- [ ] A class with no reachable public constructor warns (`WARNING_NO_PUBLIC_CONSTRUCTOR`) but the generated C# class itself carries no XML `<remarks>` doc comment naming the constraint, so the signal only reaches the library author's build log, not a consumer's IDE tooltip; the generator emits no `///` doc comments at all today, so this needs new renderer machinery rather than a small addition. Deferred by [ADR-064](docs/adr/064-forward-unsupported-declaration-diagnostics.md)'s 2026-09-07 amendment.
- [ ] **A merged extension class for an unexported receiver (e.g. `StringExtensions`) takes the namespace of whichever extension KSP visits first, rather than a deterministic one, so adding a same-receiver extension in another package silently relocates every existing extension's C# class.** ([details](docs/backlog/extension-class-namespace-order-dependent.md))
- [ ] A structural `role` field on `ForwardAbiParameter` distinguishing a generator-owned ABI slot from user data, checked once instead of string-matched at each read site, was considered and deferred when the reserved-name invariant shipped: it would replace the name lookup at roughly twenty construction sites and cannot reach `ForwardAbiContract`, which works from a rendered KotlinPoet `FunSpec` with no such field left to check. Discovered alongside [ADR-062](docs/adr/062-forward-callable-plan.md)'s 2026-09-07 amendment.
- [ ] `CirNativeImports.kt:~107` derives the extern name `Native_${method.name}` from the public method name when `externName` is null, so a legacy translator that ever produced a keyword-spelled `CirMethod.name` (they all apply `toCSharpName` after PascalCasing, so unreachable today) would leak `@` into the extern identifier; the plan path always sets `externName`. Verified by reading, discovered alongside [ADR-062](docs/adr/062-forward-callable-plan.md)'s 2026-09-07 amendment.
- [ ] **A sealed *interface* at a property position (bare or as a collection component) still skips named after [ADR-105](docs/adr/105-sealed-property-position.md), because only a sealed *class* gets the ADR-009 `FromHandle` discriminator.** `NugetProcessor.kt`'s `rootSealedClasses` filters on `classKind == CLASS` and `ForwardBridgeTypeClassifier.kt`'s `SEALED` branch fires ahead of the interface branch, so a sealed interface is a bare `sealed helper` protocol at every position; pinned by `Tier1SealedCollectionPropertyTest`, deferred by ADR-105 alongside issue [#54](https://github.com/xxfast/kotlin-native-nuget/issues/54).
- [ ] Whether a `data object` sealed subclass (`ForwardPlanSkipReason.OBJECT` kind) is admitted at a member position (return, property, or parameter) is unverified either way; no fixture carries one. Discovered alongside the nested sealed-subclass member-position fix ([ADR-009](https://github.com/xxfast/kotlin-native-nuget/blob/main/docs/adr/009-sealed-class-mapping.md)).
- [ ] A value class whose underlying type is sealed still skips named at a property position: `ForwardPropertyPlanner.kt`'s `isPlannable` `ValueClass` arm admits only String/Primitive/Enum/ObjectHandle underlyings and never sees `sealedHandle`, and the callable planner's inline rewrite (`ForwardCallablePlanner.kt:~450`) does not run for properties. Deferred by [ADR-105](docs/adr/105-sealed-property-position.md) alongside issue [#54](https://github.com/xxfast/kotlin-native-nuget/issues/54).
- [ ] An extension function whose *receiver* is a sealed base still skips: `sealedAsHandle()` rewrites every declared parameter in `ForwardCallablePlanner.kt`, but deliberately leaves the receiver alone, since a sealed receiver is a member of the ADR-009 hierarchy with its own named route. Verified by reading during [ADR-105](docs/adr/105-sealed-property-position.md)'s 2026-09-07 amendment.
- [ ] **A reference-underlying value-class constructor ignores `ctor.nativeArguments`, recomputes its overload suffix from a loop index instead of `ctor.nativeSuffix`, and hands an `IntPtr` to a class-typed positional parameter.** ([details](docs/backlog/reference-underlying-value-class-constructor-latent-defects.md))
- [ ] The legacy `buildConstructor` fallback in `CirClassTranslator.kt` (~1767-1776) spells a value-class constructor parameter type bare via `mapParamType` rather than `qualifiedElementCsType`, so a cross-package parameter type would fail CS0246 if this callable-catalog-miss path is ever actually reached; reachability is inferred from the code shape, not exercised by a fixture. Discovered alongside the cross-package value-class underlying qualification fix ([ADR-066](docs/adr/066-forward-export-reachability-closure.md)).
- [ ] `qualifiedElementCsType`'s KDoc (`CirTypeMapping.kt:184-192`) and `Issue41Bundle.kt`'s fixture KDoc still describe the "same namespace stays bare, everything else qualified" rule the #41 fix removed entirely in favour of unconditional qualification; both comments should be corrected to match the shipped behaviour. Discovered alongside the cross-package value-class underlying qualification fix ([ADR-066](docs/adr/066-forward-export-reachability-closure.md)).
...
- [ ] A property-position skip never carries the `SKIPPED_UNEXPORTED_DEPENDENCY_TYPE`/`UNDECLARED_ENUM` `include(...)` hint the way a parameter/return-position skip does: both `NestedModeOwner.mode` (module-local), `Broadcast.band` (admitted-then-refused nested dependency enum), and `NestedListenerOwner.listener` (undeclared nested interface) land on the generic `SKIPPED_UNSUPPORTED_PROPERTY` message instead. The property route's `ForwardDroppedProperty` carries no `ForwardPlanSkipReason` at all, so `NugetProcessor.kt`'s property-diagnostic producer hardcodes one hint for every type mismatch rather than branching on the reason a parameter/return-position skip already carries. Verified in the feature's own `NugetDiagnostics.json`. Discovered alongside the undeclared-enum classifier gate fix (2026-09-05, no ADR).
- [ ] A sealed-subclass **method** (`Shape.Circle.pick(): Owner.Listener?`) produced neither an export nor a diagnostic in the same spike; whether sealed-subclass methods are exported at all on the ADR-009 route was not chased, so if they are, this is the same silent-drop defect class an ordinary class method returning a sealed type used to have (closed by [ADR-009](docs/adr/009-sealed-class-mapping.md)'s 2026-09-07 amendment, which did not touch sealed-subclass methods). Verified by spike, not by a fixture, discovered alongside [ADR-040](docs/adr/040-interface-return-type-mapping.md)'s 2026-09-07 amendment.
...
- [ ] A sealed *interface*, or an out-of-scope sealed class, at a callable **return** position now also takes `SEALED_POSITION` (no `FromHandle` discriminator exists for either), so `SKIPPED_SEALED_POSITION`'s hint ("binds at return and property positions") reads wrong for that shape; previously silent, so strictly better, but the wording should branch on whether a discriminator actually exists. Verified by reading `ForwardCallablePlanner.kt` during [ADR-064](docs/adr/064-forward-unsupported-declaration-diagnostics.md)'s 2026-09-07 amendment.
- [ ] `GENERIC`, `FLOW_PROTOCOL`, and `CALLBACK_PROTOCOL` are still `droppedFromCSharp = false` legacy-route deferrals on the same "some route re-emits this" assumption that made `SEALED_PROTOCOL` silently vanish at a parameter position; whether that assumption holds at every position each reason can be classified at (a class method's generic return/parameters, a top-level `Flow` return/parameter, a lambda return) is unverified, inferred from the absence of a re-emitting arm rather than spiked. Discovered alongside [ADR-064](docs/adr/064-forward-unsupported-declaration-diagnostics.md)'s 2026-09-07 amendment.
