...
- [ ] **Reverse-bridge registration trace.** The [ADR-041](docs/adr/041-kotlin-to-csharp-call-mechanism.md) function-pointer table is unobservable: nothing can report which `nuget_{ns}_{type}_register` exports actually fired, so "did registration work?" is only answerable by bisecting. That cost a 45-minute agent round that produced zero changes, and let stale build state masquerade as a codegen bug for hours. Emit an opt-in trace (env var, e.g. `NUGET_INTEROP_TRACE=1`) from the generated C# `[ModuleInitializer]` shims (`NugetGenerateShimsTask`) and the Kotlin registration functions (`NugetGenerateBindingsTask`): log each registration as it lands, and each bridge call that finds an unregistered slot. Plugin consumers will hit the same wall, so this ships to them too
