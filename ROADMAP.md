...
- [ ] **A forward projection that matches an ABI parameter by its literal name (`handle`, `value`, `receiver`, `errorOut`, `valueOut`) silently misroutes a user parameter carrying that same name.** ([details](docs/backlog/forward-projection-matches-abi-parameter-literal-name.md))
