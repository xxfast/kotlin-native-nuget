...
| `data class`              | ← | `struct` (`readonly record struct`, etc.) | reverse only: a "Shape A" struct (exactly one public ctor whose parameters, matched case-insensitively, cover every public readable property and all stored instance state; components are primitives incl. `bool`/`char`, `string`, and bound enums) decomposes onto the wire, no struct crosses the C ABI itself, one ABI argument per component in, `void` + one out-pointer per component out; surfaces as an immutable Kotlin `data class`, zero registration slots, no handle/`close()`; Shape B (no-ctor) structs, multiple ctors, struct methods/computed properties, nested/class-typed components, generic structs, and structs in collections are deferred | [ADR-056](docs/adr/056-csharp-structs-in-kotlin.md) |
