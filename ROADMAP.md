...
- [ ] `CirClassTranslator.kt:~1086` still empties the property list for a `data object` while `SealedClassExports.kt` exports `_get_*` for a data object's public properties unconditionally, so a `data object` overriding a base `val` gets an orphan Kotlin export with no C# property behind it; harmless today (no fixture has such a property); the remaining use of `CirSealedSubclass.isDataObject`. Verified by reading, discovered alongside ADR-009's 2026-09-07 data-object amendment.
- [ ] An ineligible sealed interface's nested subclass gets two build warnings for one hierarchy: `SKIPPED_INELIGIBLE_SEALED_INTERFACE` on the parent and `SKIPPED_NESTED_DECLARATION` on the subclass itself; suppressing the second needs a rule [ADR-112](docs/adr/112-sealed-interface-mapping.md) does not specify. Discovered alongside ADR-112.
