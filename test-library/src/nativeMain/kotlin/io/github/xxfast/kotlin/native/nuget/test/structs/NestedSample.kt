...
import test.nested.Nurseries
import test.nested.Nursery
import test.structs.Collar
import test.structs.Extent
import test.structs.Litter
import test.structs.Litters
import test.structs.Nest
import test.structs.Point
import test.structs.Profile
import test.structs.Shelter
...
//       -> Kotlin test-library             NestedSample.kt (this file)
...
//                                            Test.Nested.{Nursery,Nurseries}
//
// Nursery deliberately lives in a DIFFERENT C# namespace (Test.Nested, not Test.Structs), and
// therefore a different Kotlin package (test.nested, not test.structs): its `litter`
// component is a `test.structs.Litter`. This is the only fixture shape that exercises the two
// cross-package import sites nesting creates (a Kotlin `import test.structs.Litter` in the
// generated Nursery.kt, and a C# `using Test.Structs;` in the generated shim) — both broken on
...
//
// These functions will not compile against `test-library` until kotlin-dev lands the
...
// DEPTH 2, both directions, and ACROSS a Kotlin package boundary: Nursery is `test.nested`,
// its `litter` component is `test.structs`. Compiles only if the generated Nursery.kt imports
// it.
