...
- [ ] **A keyword-named parameter on a legacy C# route (suspend, `Flow`, lambda, sealed, generic, interface-bridge) still generates invalid C#, since only the ordinary synchronous forward callable plan escapes or renames parameter names; an `error`-named parameter on the same routes is unrenamed too and collides wherever that route also declares an `error` identifier.** ([details](docs/backlog/legacy-routes-keyword-parameter-names-not-escaped.md))
...
- [ ] **An abstract class with a structurally-skipped inherited member renders it `public abstract`, so a further subclass fails CS0534.** ([details](docs/backlog/abstract-class-renders-structurally-skipped-member-abstract.md))
