...
- [ ] **The suspend legacy routes (`asyncMembers` and `suspendStateFlowMembers` in `CirClassTranslator.kt`, `SuspendFunctionExports.kt`) compose their `_async` entry point off the bare method name, so two `suspend` overloads collide on one C symbol exactly as the flow route did in [#97](https://github.com/xxfast/kotlin-native-nuget/issues/97).** Inferred from reading, not spiked. The fix is the one #97 landed: read `ForwardCallablePlanCatalog.overloadSuffix(method)` on both sides (the planner already numbers a `SUSPEND` skip).
