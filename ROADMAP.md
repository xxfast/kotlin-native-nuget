...
- [ ] **A class method returning a sealed type is silently dropped, with no diagnostic.** A top-level `fun loaded(): State` binds (the #39/#50 fixtures), but the same signature as a method on an exported class generates nothing and warns nothing: the class came out with only its constructor and `Dispose()`. Found while building the [#50](https://github.com/xxfast/kotlin-native-nuget/issues/50) fixture, established by one `packNuget` run, not chased. `NestedShapeFactory.shapeOf(): NestedShape` in `NestedShapeSample.kt` reproduces the same drop, verified by a `packNuget` run on 2026-09-05: no export, no diagnostic. `FlatShapeFactory.of(radius): FlatShape` in `FlatShapeSample.kt` reproduces the same drop a second time, verified by this item's own `packNuget` run.
...
- [ ] `Shape.Circle.listener` (a sealed-subclass property typed with a nested interface) skips on the C# side, but Kotlin still emits an orphan `@CName("shape_circle_get_listener")` export on the legacy ADR-009 route: harmless (compiles, uncalled) but a forward Kotlin/C# drift. Verified by a Tier 1 spike during the nested-interface skip item's research (`exports/SealedClassExports.kt`), discovered alongside [ADR-040](docs/adr/040-interface-return-type-mapping.md)'s 2026-09-07 amendment.
- [ ] A sealed-subclass **method** (`Shape.Circle.pick(): Owner.Listener?`) produced neither an export nor a diagnostic in the same spike; whether sealed-subclass methods are exported at all on the ADR-009 route was not chased, so if they are, this is the same silent-drop defect class as "A class method returning a sealed type is silently dropped" above. Verified by spike, not by a fixture, discovered alongside [ADR-040](docs/adr/040-interface-return-type-mapping.md)'s 2026-09-07 amendment.
