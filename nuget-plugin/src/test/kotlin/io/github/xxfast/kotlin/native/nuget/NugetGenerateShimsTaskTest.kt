...
class NugetGenerateShimsTaskTest {
  ...
  @Test
  fun `every shim carrying a ModuleInitializer starts with the auto-generated header`() {
    // That header, not a pragma, is what keeps a consumer with TreatWarningsAsErrors compiling:
    // Roslyn treats a file whose leading comment contains `<auto-generated` as generated code, and
    // CA2255 ("ModuleInitializer is only intended for application code") opts out of generated code.
    // Spiked: header present builds clean under AnalysisLevel=latest-all, header absent is
    ...
    val rir: RirFile = RirFile(
      assemblies = listOf(
        RirAssembly(
          ...
          namespaces = listOf(
            RirNamespace(name = "Test.Kennel", types = listOf(iBadge, collar, kennel, tools)),
          ),
        ),
      ),
    )

    val shims: List<GeneratedFile> = generateCSharpShims(rir, "sample")
    val initializers: List<GeneratedFile> =
      shims.filter { it.content.contains("[ModuleInitializer]") }
    assertTrue(
      initializers.size >= 5,
      "expected a shim per family (static class, class, interface, struct, runtime), got " +
          initializers.map { it.relativePath },
    )
    initializers.forEach { shim ->
      assertTrue(
        shim.content.trimStart().startsWith("// <auto-generated"),
        "${shim.relativePath} carries a [ModuleInitializer] without the <auto-generated> header, " +
            "so a consumer with TreatWarningsAsErrors gets CA2255",
      )
    }
  }
}
