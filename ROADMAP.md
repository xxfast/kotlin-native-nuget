...
- [x] **Harness guardrails in `.claude/settings.json`.** Two rules in [AGENTS.md](AGENTS.md) were honour-system, and both were violated in the very session that wrote them. A `PreToolUse` hook (`.claude/hooks/block-generated-writes.sh`) now refuses `Edit`/`Write` into `~/.nuget/packages/**` and any `**/build/**` output, and explains why rather than just blocking (`build.gradle.kts` is deliberately still writable). A `Stop` hook (`.claude/hooks/warn-stray-gradle.sh`) reports a stray `gradlew` / `GradleWorkerMain` left running in this project when an agent finishes, since an orphan holds the project lock and silently starves the next agent. It **reports rather than kills**: a build you started deliberately is indistinguishable from an agent's orphan, and killing the wrong one loses real work. It ignores the shared long-lived `GradleDaemon` and any process whose working directory is a different checkout
