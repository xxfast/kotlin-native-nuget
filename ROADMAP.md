...
- [ ] The reachability closure's dependency-type admission filters `parentDeclaration` only on the `ENUM` branch (`ForwardReachabilityClosure.kt`); a nested dependency `class`/`object` has no equivalent filter, so whether it is still declared at namespace root under its simple name is unverified either way. Discovered alongside the undeclared-enum classifier gate fix (2026-09-05, no ADR).
