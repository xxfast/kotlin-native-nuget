...
| `data class`              | ← | `struct` (`readonly record struct`, etc.) | reverse only: a "Shape A" struct has exactly one state-covering constructor whose parameters define the components; alternate public constructors become Kotlin secondary constructors backed by out-pointer bridge slots. Instance methods and get-only non-component computed properties land on the `data class` (reconstruct-on-call: N component wire args first); static methods land in a `companion object`. The value still decomposes onto the wire and has no handle/`close()`; Shape B structs, nested/class-typed components, generic structs, and structs in collections are deferred | [ADR-056](docs/adr/056-csharp-structs-in-kotlin.md), [ADR-057](docs/adr/057-csharp-overload-sets-in-kotlin.md), [ADR-014](docs/adr/014-value-class-mapping.md) |
