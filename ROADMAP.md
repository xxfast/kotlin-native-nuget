...
- [ ] A sealed *interface* at a property position (bare or as a collection component) still skips named after [ADR-105](docs/adr/105-sealed-property-position.md), because only a sealed *class* gets the ADR-009 `FromHandle` discriminator (`NugetProcessor.kt`'s `rootSealedClasses` filters on `classKind == CLASS`), and so does a value class whose underlying is sealed (`isPlannable`'s `ValueClass` arm never sees `sealedHandle`). Pinned by `Tier1SealedCollectionPropertyTest`; deferred by ADR-105 alongside issue [#54](https://github.com/xxfast/kotlin-native-nuget/issues/54).
