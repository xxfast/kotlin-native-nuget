...
- [ ] **Two exported types with the same simple name in different packages collide on their export symbol, and both a constructor and a top-level function hit this.** ([details](docs/backlog/two-exported-types-same-simple-name-different.md))
...
- [ ] **`kotlin.time.Duration` → `System.TimeSpan`.** ([details](docs/backlog/kotlin-time-duration-system-timespan.md))
