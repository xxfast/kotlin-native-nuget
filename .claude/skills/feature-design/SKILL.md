...
1. **Restate the item from its own words, with every linked ADR still closed.** The item's own words are its ROADMAP line **plus** its `docs/backlog/<slug>.md` details file when it links one; read both. One sentence: what does a consumer get? Name the **direction** (Kotlin → C# forward, C# → Kotlin reverse) and **which side declares** the type or contract versus merely consumes it.
2. **Read the item's neighbours and section heading.** `git log -S"<item text>" -- ROADMAP.md docs/backlog/` shows what arrived alongside it (history before the 2026-08-31 slim-down lives in `ROADMAP.md`'s own log; the pre-slim-down text is snapshotted in `docs/archive/roadmap.md`). If the section's stated direction contradicts your restatement, stop and ask.
3. **Check the item is reachable.** If it needs a later phase or a still-open sibling it is **blocked**: report what it needs and stop. Never build the nearest reachable thing against a blocked item's checkbox.
...
5. **Check for a staged research memo.** `ls docs/research/roadmap/` for a memo whose slug matches the item (the README there gives the format and lifecycle). If one exists and is fresh (its date is later than the last commit touching any `file:line` it cites, and every ADR it cites is still at the status it records), **skip Step 1**: go straight to Step 2 with the memo's findings, restatement and open questions, and say in the kickoff message that the memo is the research. A memo whose line numbers have drifted or whose "Decided" answers a different question is stale: re-run Step 1 and have the `research` agent rewrite the memo rather than trusting it.
6. **State the restatement to the human and dispatch Step 1 in the same message.** Do not wait for a reply: Step 2 is where the human confirms the restatement, side by side with the findings. Hard-stop before dispatching only when a check above failed (blocked item, direction contradiction, ADR scope mismatch).

Carry the restatement forward verbatim: Step 2 is judged against it, and Step 5 may close the item (delete its line and backlog file) only if what shipped matches it.
...
- **"queue ahead" items**: research can run long before implementation. Fan out one background `research` agent per item and have each write `docs/research/roadmap/<slug>.md` (format in that directory's README); present the findings once, record the human's decisions in each memo's "Open what-questions" section, and leave the items on the ROADMAP. A later run of this skill picks the memo up in Step 0 and starts at Step 2. Ten memos from one research fan-out cost about 15 minutes of wall clock (measured 2026-09-14).
...
- **Shared doc surfaces.** Every item's `documenter` edits the same lines: the phase block in ROADMAP.md, `docs/topics/supported-features.md`, `docs/adr/README.md`, and two or three topic pages. Reserve ADR numbers up front in the ledger (one per needs-ADR item, in stack order) so parallel drafts never collide on a number, and expect to resolve ROADMAP/supported-features/README conflicts by hand during assembly; they are line-adjacent inserts and deletes, mechanical but never automatic.
...
- Write the findings to `docs/research/roadmap/<slug>.md` in the README's format, whether or not the item is being built now. The memo is what a later run reads instead of re-researching; the agent's report to the main thread is a summary of it, not a substitute.
...
- **At a scope fork, recommend the right end state, not the narrowest patch** (see Rules). If the narrow option leaves a second dialect in place (a hand-written route beside the ADR-062 plan, a duplicated predicate, a template that will have to migrate later anyway), the recommendation is the option that retires it, priced honestly. Narrow wins only when both options land on the same end state. "The deferred half shares a code path" still never justifies pulling in an adjacent *capability* the restatement did not ask for.
...
- **Forward feature (Kotlin → C#)**: `csharp-dev` writes failing xunit tests in `IntegrationTests/` that define the expected API (follow existing patterns, `using var` for IDisposable), adding Kotlin sample source in `test-library/` if needed.
  - **Every new forward route adds a row to `LeakTests/LiveHandleTests.cs` (its own test process, because the counter is process-global; never move it back into `IntegrationTests/`)** in the same pass: one `[Fact]` driving the route N times inside `AssertNoLeak`/`AssertNoLeakAsync` and asserting the [ADR-120](../../../docs/adr/120-live-stableref-counter-and-leak-harness.md) handle count returns to baseline. Without the row the counter gives only accidental coverage: a leak on the new route shows up as an unexplained positive delta in whichever harness row runs later, if at all (that is how the ADR-019 suspend race was found, on Windows CI, as a flake in the `Set` parameter row). If the route has a throw path, add a fault-injection row too, on the pattern of `ListReturn_ThrowingElementFactory_ReleasesTheListHandle`. A route whose body can complete before the P/Invoke returns (a suspend function with no suspension point, a callback fired synchronously) needs a tight-loop row with thousands of iterations, because that class of leak is a race that reads +1 per thousand, not +1 per call.
  - A new renderer must mint and release through the generated `NugetHandles.retain`/`release`, never a bare `StableRef.create(...)`/`.dispose()`. An uncounted mint reads green in the harness while leaking; until the structural test on the ROADMAP exists, `kotlin-dev` greps the generated `CNameExports.kt` for bare `StableRef.create(` before reporting.
...
- **`documenter`**: hand it the feature, the ADR, the ROADMAP item, the sample/test files, and **every bug split out but not fixed** (they exist only in the implementing agents' reports; if you do not forward them they are lost). Give symptom, root cause, `file:line`, and whether verified. It owns every doc surface: the Writerside pages in `docs/topics/` (mapping table, snippets, and deleting the now-false **Limitations** line), the ROADMAP close-out (delete the item's line, its backlog file, and its `docs/research/roadmap/<slug>.md` memo; add one-line items, with `docs/backlog/` files, for the split-out bugs), the `docs/topics/supported-features.md` row with its direction glyph (`→` forward, `←` reverse, `⇄` both, Notes for asymmetry), one clause of at most 200 characters, `scripts/verify-features.sh` green, and marking the ADR `Accepted`. **Before instructing the close-out, re-read the item's text (line + backlog file) against the Step 0 restatement**: the `documenter` never saw the request and will close whatever line you name. Delivered-but-different work gets its own item, and a parent line that contradicts its sub-bullets is a mislabel to escalate, not close around. **Also hand it the name of the `LiveHandleTests` row the feature added** (or the reason none was needed, e.g. the route mints no handle); the `documenter` records the row in the diagnostics page's harness list, and a forward feature reported without one is not done, send it back to Step 3.
...
- **The restatement is a ceiling as well as a floor, but the mechanism is not.** Every other rule here pushes toward completeness; this one stops you shipping four times the right thing. Scope (which consumer capability ships) stays at the restatement: sharing a code path is not sharing a request, and ADR-075 began as a two-branch crash fix and shipped as four subsystems, widened twice on the assistant's own recommendation. Mechanism (which route the capability lands on) is the opposite: at a fork between a patch on a legacy route and the route the codebase is converging on, put **Recommended** on the end state and price both in concrete terms ("two `when` branches" against "most of an ADR"). ADR-147 recommended the patch, the human overruled it at the gate (2026-09-16), and the research had to be redone for the end state; recommend it first.
...
- **Check ROADMAP.md, `docs/backlog/`, and the archive (`docs/archive/roadmap.md`) before proposing an item**, including as a *rejected* one; settled and completed decisions live in the archive, and re-proposing one wastes the human's time.
