...
internal object ForwardCirPropertyProjection {
  ...
  private fun classExtraNatives(plan: ForwardPropertyPlan): List<CirExtraNative> = buildList {
    if (plan.getter is ForwardPropertyGetter.LegacyTwoCall) {
      ...
      add(CirExtraNative("get_${plan.kotlinName}_value", value.result.csharpWireType(), "Native_Get_${plan.kotlinName}_value", hasSyncErrorOut = true))
    }
    if (plan.setter is ForwardPropertySetter.NullableDispatch) {
      add(CirExtraNative("set_${plan.kotlinName}_null", "void", "Native_Set_${plan.kotlinName}_null", hasSyncErrorOut = true))
    }
  }
  ...
  private fun setterBody(plan: ForwardPropertyPlan, receiver: String): String {
    ...
    return when (val setter = plan.setter) {
      ...
      is ForwardPropertySetter.NullableDispatch -> buildString {
        ...
        append(checkedVoidBody(nativeName(plan, setter.value), args(plan.valueArgument("value.Value")), indent = "                "))
  ...
  private fun ForwardPropertyPlan.valueArgument(name: String = "value"): String = when (val value = type.unwrapNullable()) {
  ...
  private fun BridgeType.wireType(): ForwardAbiWireType = when (val type = unwrapNullable()) {
    BridgeType.Unit -> ForwardAbiWireType.VOID; BridgeType.String, is BridgeType.ObjectHandle, is BridgeType.Collection -> ForwardAbiWireType.POINTER
    is BridgeType.Enum -> ForwardAbiWireType.INT32; is BridgeType.Primitive -> type.kind.wireType()
  ...
  private fun BridgeType.csharpType(): String = when (this) {
    is BridgeType.Nullable -> "${type.csharpType()}?"; is BridgeType.Primitive -> kind.csharpType(); BridgeType.String -> "string"
    is BridgeType.Enum -> qualifiedName.substringAfterLast('.')
    is BridgeType.ObjectHandle -> qualifiedName.substringAfterLast('.')
    is BridgeType.Collection -> if (kind == CollectionKind.LIST) "IReadOnlyList<${requireNotNull(element).csharpType()}>" else "IList<${requireNotNull(element).csharpType()}>"
  ...
  private fun ForwardAbiWireType.csharpWireType(): String = when (this) {
    ...
    ForwardAbiWireType.INT16 -> "short"; ForwardAbiWireType.UINT16, ForwardAbiWireType.CHAR16 -> "ushort"; ForwardAbiWireType.INT32 -> "int"; ForwardAbiWireType.UINT32 -> "uint"
    ForwardAbiWireType.INT64 -> "long"; ForwardAbiWireType.UINT64 -> "ulong"; ForwardAbiWireType.FLOAT32 -> "float"; ForwardAbiWireType.FLOAT64 -> "double"; ForwardAbiWireType.STRING -> "string"; ForwardAbiWireType.POINTER -> "IntPtr"
