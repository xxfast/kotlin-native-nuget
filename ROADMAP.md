...
- [ ] A property-position skip never carries the `SKIPPED_UNEXPORTED_DEPENDENCY_TYPE`/`UNDECLARED_ENUM` `include(...)` hint the way a parameter/return-position skip does: both `NestedModeOwner.mode` (module-local) and `Broadcast.band` (admitted-then-refused nested dependency enum) land on the generic `SKIPPED_UNSUPPORTED_PROPERTY` message instead. Verified in the feature's own `NugetDiagnostics.json`. Discovered alongside the undeclared-enum classifier gate fix (2026-09-05, no ADR).
