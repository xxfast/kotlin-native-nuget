...
//       -> Kotlin sample-library             NestedSample.kt
...
// (Sample.Nested, not Sample.Structs) from the struct it nests, which is the only way to exercise
...
public class NestedStructRoundTripTests
{
    ...
    // DEPTH 2, both directions, ACROSS a Kotlin package boundary (Nursery: sample.nested, its
    // Litter component: sample.structs). Compiles only if the generated Nursery.kt imports
    // sample.structs.Litter and the generated shim `using`s Sample.Structs.
    [Fact]
