...
private fun FileSpec.Builder.addGetter(plan: ForwardPropertyPlan, call: ForwardNativeCall) {
  val builder = exportBuilder(call, plan.receiver)
  val access = plan.accessExpression()
  when (val type = plan.type) {
    ...
    is BridgeType.Primitive -> {
      ...
      builder.addCode(valueBody(access, "errorOut", primitiveDefault(type)), cOpaquePointerVar, stableRef)
    }
    BridgeType.String -> {
      builder.returns(kotlinType(type))
    ...
    is BridgeType.Nullable -> when (val inner = type.type) {
      ...
      is BridgeType.ObjectHandle -> {
        ...
        builder.addCode(nullableHandleBody(access, "errorOut"), stableRef, cOpaquePointerVar, stableRef)
      }
      else -> error("Forward property direct nullable getter is invalid for ${plan.symbol}: $inner")
    }
    is BridgeType.Enum -> {
      builder.returns(kotlinType("Int"))
      builder.addCode(valueBody("$access.ordinal", "errorOut", "0"), cOpaquePointerVar, stableRef)
    }
...
private fun FileSpec.Builder.addNullablePresenceGetter(plan: ForwardPropertyPlan, call: ForwardNativeCall) {
  val builder = exportBuilder(call, plan.receiver).returns(kotlinType("Boolean"))
  builder.addCode(valueBody("${plan.accessExpression()} != null", "errorOut", "false"), cOpaquePointerVar, stableRef)
  addFunction(builder.build())
}

private fun FileSpec.Builder.addNullableValueGetter(plan: ForwardPropertyPlan, call: ForwardNativeCall) {
  val type = (plan.type as BridgeType.Nullable).type as BridgeType.Primitive
  val builder = exportBuilder(call, plan.receiver).returns(kotlinType(type))
  builder.addCode(valueBody("${plan.accessExpression()}!!", "errorOut", primitiveDefault(type)), cOpaquePointerVar, stableRef)
  addFunction(builder.build())
}
...
private fun FileSpec.Builder.addSetter(
  ...
  val builder = exportBuilder(call, plan.receiver, includeError = false)
  if (assignsNull != true) {
    val valueType = requireNotNull(call.parameters.firstOrNull { it.name == "value" }?.transfer?.type) {
  ...
  val expression = plan.accessExpression()
  val assignment = when {
    assignsNull == true -> "$expression = null"
    else -> "$expression = ${plan.valueExpression()}"
  }
...
private fun exportBuilder(
  ...
  val builder = FunSpec.builder("export_${call.exportName}").addAnnotation(cNameAnnotation(call.exportName))
  when (receiver) {
    is ForwardPropertyReceiver.Handle -> builder.addParameter("handle", cOpaquePointer)
    is ForwardPropertyReceiver.Value -> builder.addParameter("receiver", kotlinInputType(receiver.type))
...
private fun ForwardPropertyPlan.accessExpression(): String = when (val receiver = receiver) {
  is ForwardPropertyReceiver.Handle -> "handle.asStableRef<${receiver.owner}>().get().$kotlinName"
  is ForwardPropertyReceiver.Value -> when (val type = receiver.type) {
    is BridgeType.ObjectHandle -> "receiver.asStableRef<${type.qualifiedName}>().get().$kotlinName"
    else -> "receiver.$kotlinName"
  }
  is ForwardPropertyReceiver.Static -> receiver.owner?.let { "$it.$kotlinName" } ?: kotlinName
}

private fun ForwardPropertyPlan.valueExpression(): String = when (val type = type) {
  is BridgeType.Nullable -> when (val inner = type.type) {
    is BridgeType.Primitive, BridgeType.String -> "value"
...
private fun kotlinInputType(type: BridgeType): TypeName = when (type) {
  ...
  is BridgeType.ObjectHandle, is BridgeType.Collection -> cOpaquePointer.copy(nullable = type is BridgeType.Nullable)
...
private fun kotlinType(type: BridgeType): TypeName = when (type) {
  ...
  is BridgeType.Primitive -> kotlinType(
    when (type.kind) {
      PrimitiveKind.BOOLEAN -> "Boolean"; PrimitiveKind.BYTE -> "Byte"; PrimitiveKind.UBYTE -> "UByte"
      PrimitiveKind.SHORT -> "Short"; PrimitiveKind.USHORT -> "UShort"; PrimitiveKind.INT -> "Int"
      PrimitiveKind.UINT -> "UInt"; PrimitiveKind.LONG -> "Long"; PrimitiveKind.ULONG -> "ULong"
      PrimitiveKind.FLOAT -> "Float"; PrimitiveKind.DOUBLE -> "Double"
    },
  )
  BridgeType.String -> kotlinType("String")
...
private fun primitiveDefault(type: BridgeType.Primitive): String = when (type.kind) {
  ...
  PrimitiveKind.UBYTE -> "0.toUByte()"; PrimitiveKind.USHORT -> "0.toUShort()"
  PrimitiveKind.UINT -> "0u"; PrimitiveKind.ULONG -> "0uL"
  PrimitiveKind.FLOAT -> "0.0f"; PrimitiveKind.DOUBLE -> "0.0"
}

private fun unitBody(invocation: String, error: String): String = buildString {
  appendLine("try {"); appendLine("  $invocation"); appendLine("} catch (e: Throwable) {")
  appendLine("  if ($error != null) {")
  appendLine("    $error.reinterpret<%T>().pointed.value = %T.create(")
  appendLine("      buildError(e)"); appendLine("    ).asCPointer()"); appendLine("  }"); append("}")
}

private fun valueBody(invocation: String, error: String, fallback: String): String = buildString {
  appendLine("return try {"); appendLine("  $invocation"); appendLine("} catch (e: Throwable) {")
  appendLine("  if ($error != null) {")
  appendLine("    $error.reinterpret<%T>().pointed.value = %T.create(")
  appendLine("      buildError(e)"); appendLine("    ).asCPointer()"); appendLine("  }")
  appendLine("  $fallback"); append("}")
}

private fun handleBody(invocation: String, error: String): String = buildString {
  appendLine("return try {"); appendLine("  %T.create($invocation).asCPointer()"); appendLine("} catch (e: Throwable) {")
  appendLine("  if ($error != null) {")
  appendLine("    $error.reinterpret<%T>().pointed.value = %T.create(")
  appendLine("      buildError(e)"); appendLine("    ).asCPointer()"); appendLine("  }")
  appendLine("  null"); append("}")
}

private fun nullableHandleBody(invocation: String, error: String): String = buildString {
  appendLine("return try {"); appendLine("  val result = $invocation")
