...
- [ ] Overload sets: revisit the ADR-043 exclusion with a deterministic disambiguation scheme. Kotlin *has* overloading, so the ceiling is only export-symbol uniqueness (mirror of ADR-034's collision rule, inverted). This also unblocks structs with multiple public constructors (deferred by [ADR-056](docs/adr/056-csharp-structs-in-kotlin.md) Decision 3a), including `System.Drawing.Point` (`Point(int,int)`, `Point(Size)`, `Point(int)`)
