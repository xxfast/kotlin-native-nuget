...
- [ ] **A keyword-named parameter on a legacy C# route (suspend, `Flow`, lambda, sealed, generic, interface-bridge) still generates invalid C#, since only the ordinary synchronous forward callable plan escapes parameter names.** ([details](docs/backlog/legacy-routes-keyword-parameter-names-not-escaped.md))
