...
class CirRenderer {
  ...
  private fun StringBuilder.renderSuspendFuncHelper(helper: CirSuspendFuncHelper) {
    ...
    for (arity in helper.arities.sorted()) {
      ...
      if (arity == 0) {
        ...
        appendLine("                    string stackTrace = NugetErrorNative.StackTrace(errorPtr);")
        appendLine("                    NugetMarshal.Dispose(errorPtr);")
        appendLine("                    t.SetException(new KotlinException(kotlinType, msg, stackTrace));")
        appendLine("                }")
        appendLine("                else")
        appendLine("                {")
  ...
  private fun StringBuilder.renderErrorHelper(helper: CirErrorHelper) {
    ...
    appendLine()
    appendLine("        public KotlinException(string kotlinType, string message, string kotlinStackTrace) : base(message)")
    appendLine("        {")
  ...
  private fun StringBuilder.renderFlowHelper(helper: CirFlowHelper) {
    ...
    appendLine("                string stackTrace = NugetErrorNative.StackTrace(errorPtr);")
    appendLine("                NugetMarshal.Dispose(errorPtr);")
    appendLine("                _channel.Writer.TryComplete(new KotlinException(kotlinType, msg, stackTrace));")
    appendLine("            };")
    appendLine()
