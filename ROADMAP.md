...
- [ ] The method-name C# keyword escape is split across layers: `ForwardCallablePlanner.kt:995` adds the `@`, `ForwardCirPlanProjection.kt:195`/`:258` strip it back with `removePrefix("@")`; now that a render-time `csharpParameterName()` helper exists for parameters, the method-name escape should move to render time too for consistency. Cosmetic, not chased. Discovered alongside issue [#65](https://github.com/xxfast/kotlin-native-nuget/issues/65).
