...
| constructor | ⇄ | `new Foo(...)` | → trailing defaults synthesize one omitting overload per suffix length; a class with no bridgeable constructor is factory-only and says so · ← each public `.ctor` becomes a secondary constructor | [Classes and objects](classes-and-objects.md) · [Instance members](instance-members.md) |
...
| function/method default parameters | → | omitting overload set | A trailing run of defaults synthesizes one omitting overload per suffix length, on all five function routes. A default followed by a required parameter synthesizes nothing. | [Classes and objects](classes-and-objects.md) |
