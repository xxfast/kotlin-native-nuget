...
**How the reverse direction works.** The Gradle plugin resolves the NuGet package, extracts its API to `reverse-ir.json`, and generates a Kotlin stub plus the C# registration shim that wires function pointers into the Kotlin/Native library at startup. `packNuget` produces a real `.nupkg` that merges the reverse shims into `contentFiles` and pins the bound package at its exact resolved version, so a consumer needs only a single `<PackageReference>` (demonstrated by the `test-library` → `MimeMapping` round trip exercised from `IntegrationTests`, [ADR-050](docs/adr/050-end-to-end-packaging-integration.md)). Reverse support includes primitives and strings, enums, opaque object handles, instance/static members, both Shape A (constructor-covered) and Shape B (public fields/settable auto-properties) structs including nested struct components (a struct-typed component may itself be a struct, at any depth), methods and computed properties on the struct, and ordinary method/constructor overloads. See [ROADMAP.md](ROADMAP.md) for the remaining surface.
...
| Kotlin                    | ⇄ | C#               | Notes                                                                                                                                                                                                                                                                                                               | ADRs                                                                                                                              |
|---------------------------|:-:|------------------|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|-----------------------------------------------------------------------------------------------------------------------------------|
| `class`                   | ⇄ | `class`          | → `: IDisposable`, StableRef + opaque pointer · ← `AutoCloseable` wrapper, opaque `GCHandle` released via `Cleaner` (deterministically via `close()` / `use {}`), C# statics land in a `companion object` and instance members on the wrapper itself; an instance member whose Kotlin name would collide with the wrapper's own `handle`/`close`/`cleaner` is skipped with a warning; object return/param nullability is metadata-driven per `NullableAttribute` (`Foo` vs `Foo?`), new wrapper per crossing (identity equality, no caching) | [ADR-005](docs/adr/005-object-return-semantics.md), [ADR-051](docs/adr/051-csharp-objects-as-opaque-handles.md), [ADR-053](docs/adr/053-nullable-reference-types-in-kotlin.md) |
| constructor               | ⇄ | `new Foo(...)`   | → Kotlin constructor surfaces as C# `new` · ← every bridgeable public instance `.ctor` becomes a Kotlin secondary `constructor(...)`; overloads retain their parameter types and use signature-derived internal bridge identities                                                                                     | [ADR-052](docs/adr/052-csharp-instance-constructors-in-kotlin.md), [ADR-057](docs/adr/057-csharp-overload-sets-in-kotlin.md) |
...
| `object`                  | ⇄ | `static class`   | → singleton, `object` in `sealed`; object methods are PascalCased with marshalled returns (e.g. a `String` return → `string`), exactly like a class method · ← consumed static class → Kotlin `object`, with static methods and properties | [ADR-007](docs/adr/007-top-level-function-class-naming.md), [ADR-048](docs/adr/048-kotlin-stub-generation-from-reverse-ir.md), [ADR-049](docs/adr/049-csharp-registration-shim-generation.md), [ADR-060](docs/adr/060-adversarial-forward-fixture.md) |
| `data object` (in sealed) | → | sealed subclass  | with `ToString`                                                                                                                                                                                                                                                                                                     |                                                                                                                                   |
| `enum class`              | ⇄ | `enum`           | → with extension methods · ← public top-level, default-`int`, non-`[Flags]` enums with unique contiguous `0..N-1` values become ordinal-backed Kotlin `enum class` values; nullable, nested, sparse, aliased, collection, and generic enum positions are not built yet | [ADR-006](docs/adr/006-enum-mapping.md) |
| `data class`              | ← | `struct` (`readonly record struct`, etc.) | reverse only: a "Shape A" struct has exactly one state-covering constructor whose parameters define the components, in constructor order; a "Shape B" struct (no such constructor; public settable fields and/or `set`/`init` auto-properties instead) draws the same components from C# declaration order and reconstructs with an object initializer (`new T { A = a, B = b }`) instead of `new T(a, b)`: both shapes share the same wire format and `data class` surface, and a Shape B struct binds no constructors at all. Alternate public constructors (Shape A only) become Kotlin secondary constructors backed by out-pointer bridge slots. Instance methods and get-only non-component computed properties land on the `data class` (reconstruct-on-call: N component wire args first); static methods land in a `companion object`. The value still decomposes onto the wire and has no handle/`close()`. A component may itself be a struct, at any depth: the wire flattens it recursively, depth-first pre-order (ABI name is the `_`-joined path), while the Kotlin surface stays nested, so structural equality and `copy()` compose across the nesting; all four Shape A/B nesting combinations work, including across a Kotlin package boundary. A member whose flattened ABI arity exceeds Kotlin/Native's 22-argument `CFunction.invoke` ceiling is skipped with a `skipped_abi_arity_limit` diagnostic instead of generating Kotlin that fails to compile. Class-typed components, generic structs, and structs in collections stay deferred, as do alternate constructors and manual (non-auto) settable properties on a Shape B struct | [ADR-056](docs/adr/056-csharp-structs-in-kotlin.md), [ADR-057](docs/adr/057-csharp-overload-sets-in-kotlin.md), [ADR-058](docs/adr/058-csharp-shape-b-structs-in-kotlin.md), [ADR-059](docs/adr/059-nested-struct-components-in-kotlin.md), [ADR-014](docs/adr/014-value-class-mapping.md) |
