...
- [ ] **Put the ranked remediation guidance back into `NugetRegistry.notRegistered()`.** [ADR-054](docs/adr/054-reverse-bridge-registration-observability.md) specified that this message would rank the likely causes ("in order of likelihood: 1. stale build state, 2. ..."), because the entire point of the feature is that the *next* person diagnoses this in seconds. What shipped is terser: one sentence naming the counts and the missing types. That tells you **what** but not **why**, and "your consumer's `obj/project.assets.json` is stale" is the answer that would have saved the ADR-053 hunt. It is one string in one generated file, and it is the highest-value sentence in the whole feature. The symptom-to-cause table in [AGENTS.md](AGENTS.md) has the content to lift
