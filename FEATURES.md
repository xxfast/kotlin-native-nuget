...
| `class` (instance, in handle positions) | `class` wrapper (`AutoCloseable`) | opaque `GCHandle` crossing as `COpaquePointer`; released automatically via `Cleaner`, deterministically via optional idempotent `close()`/`use {}`; statics in `companion object` ([ADR-051](docs/adr/051-csharp-objects-as-opaque-handles.md)) |
| object return / parameter       | `Foo?` return / `Foo` param | `IntPtr.Zero` ⇄ `null` on returns; new wrapper per crossing, equality is wrapper identity (no caching, mirror of [ADR-005](docs/adr/005-object-return-semantics.md)) ([ADR-051](docs/adr/051-csharp-objects-as-opaque-handles.md)) |
